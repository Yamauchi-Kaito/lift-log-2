...
export default function TeamCreateScreen({ onBack, onCreate, saving, error }: { onBack: () => void; onCreate: (name: string, role: string) => Promise<boolean>; saving: boolean; error: string | null }) {
  const [name, setName] = useState("")
  const [role, setRole] = useState("")
  return <main style={pageStyle}><div style={contentStyle}><header style={headerStyle}><button onClick={onBack} disabled={saving} style={backStyle}>‹</button><div><p style={eyebrowStyle}>TEAM WORKSPACE</p><h1 style={titleStyle}>チームを作成</h1></div></header><div style={{ padding: "24px" }}><p style={labelStyle}>チーム名</p><input autoFocus disabled={saving} value={name} onChange={(event) => setName(event.target.value)} placeholder="例：朝活チーム" style={inputStyle} /><p style={{ ...labelStyle, marginTop: 24 }}>自分の役職 <span style={{ color: "#666", fontWeight: 400 }}>任意</span></p><input disabled={saving} value={role} onChange={(event) => setRole(event.target.value)} placeholder="例：コーチ" style={inputStyle} /><p style={{ color: "#777", fontSize: 12, lineHeight: 1.6, marginTop: 12 }}>作成者はオーナーになります。</p>{error && <p role="alert" style={{ color: "#f09a9a", fontSize: 12, lineHeight: 1.5, marginTop: 16 }}>{error}</p>}</div><footer style={footerStyle}><button disabled={!name.trim() || saving} onClick={() => void onCreate(name.trim(), role.trim())} style={{ ...createStyle, opacity: name.trim() && !saving ? 1 : .35 }}>{saving ? "作成中..." : "チームを作成"}</button></footer></div></main>
}
