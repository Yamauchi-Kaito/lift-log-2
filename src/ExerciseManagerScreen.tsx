...
export default function ExerciseManagerScreen({ exercises, menus, loading, saving, error, onBack, onSave, onDelete }: { exercises: RegisteredExercise[]; menus: TrainingMenu[]; loading: boolean; saving: boolean; error: string | null; onBack: () => void; onSave: (exercise: RegisteredExercise, isNew: boolean) => Promise<boolean>; onDelete: (id: ExerciseId) => Promise<boolean> }) {
  ...
  return <main style={pageStyle}><div style={contentStyle}>
    <header style={headerStyle}><button onClick={onBack} style={backStyle}>‹</button><div style={{ flex: 1 }}><p style={eyebrowStyle}>EXERCISES</p><h1 style={{ fontFamily: "Outfit", fontSize: 22, fontWeight: 700 }}>種目を管理</h1></div><button onClick={() => setEditing({ id: "new", name: "", kind: "器具" })} disabled={saving} style={{ ...addTopStyle, opacity: saving ? .5 : 1 }}>＋ 追加</button></header>
    <div style={{ padding: "20px 24px 36px", flex: 1, overflowY: "auto" }}>
      <p style={{ color: "#888", fontSize: 13, marginBottom: 16 }}>登録済みの種目</p>
      {error && <p role="alert" style={errorStyle}>{error}</p>}
      {loading ? <p style={statusStyle}>読み込み中...</p> : exercises.map((exercise) => <section key={exercise.id} style={itemStyle}><div style={{ flex: 1 }}><h2 style={{ fontFamily: "Outfit", fontSize: 16, fontWeight: 700, marginBottom: 6 }}>{exercise.name}</h2><span style={typeStyle}>{exercise.kind}</span><p style={{ color: "#777", fontSize: 11, marginTop: 7 }}>{exercise.kind === "器具" ? "重量入力を使用" : "重量入力なし"}</p></div><div style={{ display: "flex", flexDirection: "column", gap: 7 }}><button onClick={() => setEditing(exercise)} disabled={saving} style={{ ...editStyle, opacity: saving ? .5 : 1 }}>編集</button><button onClick={() => setDeleting(exercise)} disabled={saving} style={{ ...deleteStyle, opacity: saving ? .5 : 1 }}>削除</button></div></section>)}
    </div>
    {editing && <ExerciseEditor exercise={editing} isNew={editing.id === "new"} saving={saving} onBack={() => setEditing(undefined)} onSave={async (exercise, isNew) => { if (await onSave(exercise, isNew)) setEditing(undefined) }} />}
    {deleting && <div style={overlayStyle}><section style={dialogStyle}><p style={{ fontFamily: "Outfit", fontSize: 18, fontWeight: 700, marginBottom: 9 }}>種目を削除しますか？</p><p style={{ color: "#888", fontSize: 13, lineHeight: 1.55, marginBottom: usedIn.length ? 10 : 22 }}>「{deleting.name}」を登録済み種目から削除します。過去のトレーニング履歴は削除されません。</p>{usedIn.length > 0 && <p style={warningStyle}>現在「{usedIn.map((menu) => menu.name).join("、")}」で使用中です。メニュー内の既存記録は残ります。</p>}<div style={{ display: "flex", gap: 9 }}><button onClick={() => setDeleting(undefined)} disabled={saving} style={{ ...dialogButtonStyle, background: "#202020", color: "#ddd", opacity: saving ? .5 : 1 }}>キャンセル</button><button onClick={async () => { if (await onDelete(deleting.id)) setDeleting(undefined) }} disabled={saving} style={{ ...dialogButtonStyle, background: "#d94b4b", color: "#fff", opacity: saving ? .5 : 1 }}>{saving ? "削除中..." : "削除する"}</button></div></section></div>}
  </div></main>
}

function ExerciseEditor({ exercise, isNew, saving, onBack, onSave }: { exercise: RegisteredExercise; isNew: boolean; saving: boolean; onBack: () => void; onSave: (exercise: RegisteredExercise, isNew: boolean) => Promise<void> }) {
  const [name, setName] = useState(exercise.name)
  const [kind, setKind] = useState(exercise.kind)
  return <main style={{ ...pageStyle, position: "fixed", inset: 0, zIndex: 20 }}><div style={contentStyle}><header style={headerStyle}><button onClick={onBack} disabled={saving} style={backStyle}>‹</button><div><p style={eyebrowStyle}>EXERCISES</p><h1 style={{ fontFamily: "Outfit", fontSize: 21, fontWeight: 700 }}>{isNew ? "種目を追加" : "種目を編集"}</h1></div></header><div style={{ padding: "24px", flex: 1 }}><p style={labelStyle}>種目名</p><input autoFocus value={name} disabled={saving} onChange={(event) => setName(event.target.value)} placeholder="例：ダンベルカール" style={inputStyle} /><p style={{ ...labelStyle, marginTop: 28 }}>種目タイプ</p><div style={{ display: "flex", gap: 9 }}>{(["自重", "器具"] as const).map((value) => <button key={value} onClick={() => setKind(value)} disabled={saving} style={{ ...typeButtonStyle, borderColor: kind === value ? "#c8ff00" : "#333", background: kind === value ? "#1b2500" : "#202020", color: kind === value ? "#c8ff00" : "#aaa" }}>{value}</button>)}</div><p style={{ color: "#777", fontSize: 12, lineHeight: 1.5, marginTop: 12 }}>{kind === "器具" ? "この種目では重量入力を使用します。" : "この種目では重量入力を基本的に使用しません。"}</p></div><footer style={footerStyle}><button onClick={onBack} disabled={saving} style={{ ...saveStyle, flex: 1, background: "#202020", border: "1px solid #333", color: "#ddd", opacity: saving ? .5 : 1 }}>キャンセル</button><button disabled={!name.trim() || saving} onClick={() => void onSave({ ...exercise, name: name.trim(), kind }, isNew)} style={{ ...saveStyle, flex: 1, opacity: name.trim() && !saving ? 1 : .35 }}>{saving ? <Spinner>保存中...</Spinner> : "保存"}</button></footer></div></main>
}
...
const statusStyle = { color: "#888", fontSize: 13, padding: "20px 0" } as const
const errorStyle = { padding: 11, marginBottom: 14, border: "1px solid #5a3030", borderRadius: 9, background: "#281818", color: "#f09a9a", fontSize: 12, lineHeight: 1.5 } as const
