...
export type RegisteredExercise = { id: number; name: string; kind: "自重" | "器具" }
export type MenuExercise = { id: number; exerciseId: number; name: string; sets: number }
export type TrainingMenu = { id: number; name: string; exercises: MenuExercise[] }

export default function MenuEditorScreen({ menu, registeredExercises, onBack, onSave, onDelete }: { menu?: TrainingMenu; registeredExercises: RegisteredExercise[]; onBack: () => void; onSave: (menu: TrainingMenu, newExercises: RegisteredExercise[]) => void; onDelete: (id: number) => void }) {
  ...
  const addExercise = (exercise: RegisteredExercise) => { if (!items.some((item) => item.exerciseId === exercise.id)) setItems((current) => [...current, { id: Date.now(), exerciseId: exercise.id, name: exercise.name, sets: 3 }]); setPickerOpen(false) }
  const updateSets = (id: number, value: number) => setItems((current) => current.map((item) => item.id === id ? { ...item, sets: Math.max(1, value || 1) } : item))
  ...
  const save = () => { if (!menuName.trim() || !items.length) return; onSave({ id: menu?.id ?? Date.now(), name: menuName.trim(), exercises: items }, createdExercises) }
  return <main style={pageStyle}><div style={contentStyle}>
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "22px 24px 126px" }}>
      ...
      {menu && <button onClick={() => setConfirmDelete(true)} style={deleteMenuStyle}>このメニューを削除</button>}
    </div>
    <footer style={footerStyle}><button onClick={onBack} style={{ ...saveStyle, flex: 1, background: "#202020", border: "1px solid #333", color: "#ddd" }}>キャンセル</button><button disabled={!menuName.trim() || !items.length} onClick={save} style={{ ...saveStyle, flex: 1.15, opacity: menuName.trim() && items.length ? 1 : 0.35 }}>保存</button></footer>
    {confirmDelete && <div style={overlayStyle}><section style={dialogStyle}><p style={{ fontFamily: "Outfit", fontSize: 18, fontWeight: 700, marginBottom: 9 }}>メニューを削除しますか？</p><p style={{ color: "#888", fontSize: 13, lineHeight: 1.5, marginBottom: 22 }}>「{menuName}」を削除します。過去のトレーニング履歴は削除されません。</p><div style={{ display: "flex", gap: 9 }}><button onClick={() => setConfirmDelete(false)} style={{ ...dialogButtonStyle, background: "#202020", color: "#ddd" }}>キャンセル</button><button onClick={() => onDelete(menu!.id)} style={{ ...dialogButtonStyle, background: "#d94b4b", color: "#fff" }}>削除する</button></div></section></div>}
  </div></main>
}
