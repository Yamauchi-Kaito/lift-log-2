...
export default function SettingsScreen({ onHome, onQuick, onHistory, onMenuEditor, onExerciseManager, onWorkspaceManager, restEnabled, onRestEnabled, restSeconds, onRestSeconds, incomingInvitations, acceptingInvitationId, invitationError, onAcceptInvitation, displayName, displayNameSaving, onSaveDisplayName, user, onSignOut }: { onHome: () => void; onQuick: () => void; onHistory: () => void; onMenuEditor: () => void; onExerciseManager: () => void; onWorkspaceManager: () => void; restEnabled: boolean; onRestEnabled: (value: boolean) => void; restSeconds: number; onRestSeconds: (value: number) => void; incomingInvitations: IncomingInvitation[]; acceptingInvitationId: string | null; invitationError: string | null; onAcceptInvitation: (id: string) => Promise<boolean>; displayName: string; displayNameSaving: boolean; onSaveDisplayName: (value: string) => Promise<string | null>; user: User; onSignOut: () => Promise<void> }) {
  ...
  return <main style={pageStyle}><div style={contentStyle}>
    <div style={{ flex: 1, overflowY: "auto", padding: "48px 24px 108px" }}>
      ...
      <Section title="体重の目標"><WeightGoalSettings userId={user.id} /></Section>
...
function WeightGoalSettings({ userId }: { userId: string }) { const [type, setType] = useState("maintain"); const [weight, setWeight] = useState(""); const [saving, setSaving] = useState(false); const [message, setMessage] = useState<string | null>(null); const [error, setError] = useState<string | null>(null); useEffect(() => { let cancelled = false; void supabase.from("body_weight_goals").select("target_weight_kg, goal_type").eq("user_id", userId).maybeSingle().then(({ data, error: loadError }) => { if (cancelled || loadError || !data) return; setType(data.goal_type); setWeight(data.target_weight_kg == null ? "" : Number(data.target_weight_kg).toFixed(1)) }); return () => { cancelled = true } }, [userId]); const save = async () => { const target = weight.trim() === "" ? null : Number(weight); if (target != null && (!Number.isFinite(target) || target < 20 || target > 400)) return setError("目標体重は20〜400kgの範囲で入力してください"); setSaving(true); setError(null); setMessage(null); const { error: saveError } = await supabase.from("body_weight_goals").upsert({ user_id: userId, target_weight_kg: target, goal_type: type, updated_at: new Date().toISOString() }, { onConflict: "user_id" }); setSaving(false); if (saveError) return setError(`目標を保存できませんでした: ${saveError.message}`); setMessage("目標を保存しました。"); }; return <div style={{ padding: 16 }}><p style={detailStyle}>体重の分析画面に表示します。</p><div style={{ display: "flex", gap: 8, marginTop: 12 }}><label style={{ flex: 1, color: "#777", fontSize: 11 }}>目的<select aria-label="体重管理の目的" value={type} onChange={(event) => setType(event.target.value)} style={displayNameInputStyle}><option value="cut">減量</option><option value="maintain">維持</option><option value="bulk">増量</option></select></label><label style={{ flex: 1, color: "#777", fontSize: 11 }}>目標体重 kg<input aria-label="目標体重" inputMode="decimal" type="number" min="20" max="400" step="0.1" value={weight} onChange={(event) => setWeight(event.target.value)} style={displayNameInputStyle} /></label></div><button disabled={saving} onClick={() => void save()} style={{ ...displayNameSaveStyle, width: "100%", marginTop: 12, opacity: saving ? .45 : 1 }}>{saving ? "保存中..." : "目標を保存"}</button>{message && <p role="status" style={{ marginTop: 10, color: "#c8ff00", fontSize: 12 }}>{message}</p>}{error && <p role="alert" style={{ marginTop: 10, color: "#f09a9a", fontSize: 12 }}>{error}</p>}</div> }
...
function NavButton({ onClick, icon, label, active }: { onClick?: () => void; icon: string; label: string; active?: boolean }) { const color = active ? "#c8ff00" : "#505050"; return <button onClick={onClick} aria-label={label} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4, padding: icon === "plus" ? "0 0 2px" : "6px 0", background: "none", border: "none", color, cursor: "pointer" }}>{icon === "plus" ? <div style={{ width: 48, height: 48, display: "grid", placeItems: "center", borderRadius: 14, background: "#222", border: "1px solid #333", marginTop: -10, color: "#f0f0f0", fontSize: 22 }}>＋</div> : <NavIcon icon={icon} />}<span style={{ fontFamily: "Inter", fontSize: 10, fontWeight: active ? 600 : 400, letterSpacing: "0.04em" }}>{label}</span></button> }
