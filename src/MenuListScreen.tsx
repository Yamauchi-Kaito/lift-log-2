...
export default function MenuListScreen({ menus, onBack, onCreate, onEdit, onStart }: { menus: TrainingMenu[]; onBack: () => void; onCreate: () => void; onEdit: (menu: TrainingMenu) => void; onStart: (menu: TrainingMenu) => void }) {
  return <main style={pageStyle}><div style={contentStyle}>
    ...
    <div style={{ padding: "22px 24px 36px", flex: 1, overflowY: "auto" }}>
      ...
      {menus.map((menu) => { const totalSets = menu.exercises.reduce((sum, exercise) => sum + exercise.sets, 0); return <section key={menu.id} style={cardStyle}>
      ...
      {menus.length === 0 && <p style={{ color: "#777", textAlign: "center", padding: "36px 0" }}>メニューがまだありません</p>}
      <button onClick={onCreate} style={createStyle}>＋ メニューを作成</button>
    </div>
  </div></main>
}
